...
export function jsonToGo(
              ...
              if (!(keyname in allFields)) {
                ...
                if (!areSameType(existingValue, currentValue)) {
                  if (existingValue !== null) {
                    ...
                    console.warn(
                      `Warning: key "${keyname}" uses multiple types. Defaulting to type "any".`
                    );
                  }
                ...
                if (areSameType(currentValue, 1))
                  allFields[keyname].value = findBestValueForNumberType(
                    existingValue,
                    currentValue
                  );
  ...
  function findBestValueForNumberType(existingValue, newValue) {
    ...
    console.error(
      `Error: something went wrong with findBestValueForNumberType() using the values: '${newValue}' and '${existingValue}'`
    );
    console.error(
      "       Please report the problem to https://github.com/mholt/json-to-go/issues"
    );
    return null; // falls back to goType "any"
  }
...
//       if (argument === "big")
//         console.warn(
//           `Warning: The argument '${argument}' has been deprecated and has no effect anymore`
//         );
//       else {
